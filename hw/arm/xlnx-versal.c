...
static void versal_create_pmcint(Versal *s, uint64_t addr)
{
    DeviceState *dev;
    SysBusDevice *sbd;

    dev = qdev_new(TYPE_XILINX_PMC_INT_REGS);
    object_property_add_child(OBJECT(s), "pmc-int", OBJECT(dev));
    sbd = SYS_BUS_DEVICE(dev);
    sysbus_realize_and_unref(sbd, &error_fatal);
...
static void versal_create_lpd_slcr(Versal *s, uint64_t addr)
{
    DeviceState *dev;
    SysBusDevice *sbd;

    dev = qdev_new(TYPE_XILINX_LPD_SLCR);
    object_property_add_child(OBJECT(s), "lpd_slcr", OBJECT(dev));
    sbd = SYS_BUS_DEVICE(dev);
    sysbus_realize_and_unref(sbd, &error_fatal);
...
static void versal_realize_common(Versal *s)
{
    ...
    if (map->pmc_int_csr) {
        versal_create_pmcint(s, map->pmc_int_csr);
    }
